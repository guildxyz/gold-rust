...
impl EditionPda {
    pub fn new(edition_type: EditionType, auction_id: &[u8]) -> Self {
        let (mint, holding) = match edition_type {
            EditionType::Master => {
                let (mint, _) =
                    Pubkey::find_program_address(&get_master_mint_seeds(auction_id), &crate::ID);
                let (holding, _) =
                    Pubkey::find_program_address(&get_master_holding_seeds(auction_id), &crate::ID);
                (mint, holding)
            }
            EditionType::Child(next_edition) => {
                let (mint, _) = Pubkey::find_program_address(
                    &get_child_mint_seeds(&next_edition.to_le_bytes(), auction_id),
                    &crate::ID,
                );
                let (holding, _) = Pubkey::find_program_address(
                    &get_child_holding_seeds(&next_edition.to_le_bytes(), auction_id),
                    &crate::ID,
                );
                (mint, holding)
            }
