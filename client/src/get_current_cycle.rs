...
use agsol_gold_contract::state::{AuctionId, AuctionRootState};
...
#[cfg(test)]
mod test {
    use super::{get_current_cycle, RpcClient};
    ...
    async fn get_current_cycle_test() {
        ...
        // unwrap ensures that the result is properly deserialized
        get_current_cycle(&mut client, &auction_id).await.unwrap();
    }
}
